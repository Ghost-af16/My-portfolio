  ...
  return (
    <section id="portfolio" className="py-24 px-4 md:px-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-semibold text-heading mb-2">
            Things I&apos;ve Built
          </h2>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <Filter className="w-4 h-4 text-faint mr-2 md:hidden" aria-hidden="true" />
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm transition-all duration-300 border ${
                  activeFilter === filter
                    ...
                    : "bg-transparent text-muted border-border hover:border-border-accent hover:text-heading"
                }`}
              >
                {filter}
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {projects.map((project) => {
          ...
          return (
            <div key={project.id} className={visible ? "" : "hidden"} aria-hidden={!visible}>
      ...
      {selectedProject?.linkType === "external" ? (
        <ProjectDetailModal project={selectedProject} onClose={() => setSelectedProject(null)} />
      ) : (
        <EmbedModal project={selectedProject} onClose={() => setSelectedProject(null)} />
      )}
    </section>
  );
}
