...
describe("portfolio.config.ts schema validation", () => {
  ...
  it("contains Viraj's public identity and links", () => {
    ...
    expect(config.gallery).toHaveLength(7);
  });

  it("uses local, uniquely described gallery images", () => {
    const gallery = config.gallery ?? [];
    expect(gallery.every((item) => item.src.startsWith("/"))).toBe(true);
    expect(gallery.every((item) => item.alt.length >= 12)).toBe(true);
    expect(new Set(gallery.map((item) => item.alt)).size).toBe(gallery.length);
    expect(gallery.every((item) => /^(I |My )/.test(item.alt))).toBe(true);
  });

  it("contains the approved first-person experience timeline", () => {
    expect(config.sections.experience).toBe(true);
    expect(config.experiences).toHaveLength(5);
    expect(config.experiences.every((item) => item.description.startsWith("I "))).toBe(true);
    expect(config.experiences[0].url).toBe("https://collageox.onrender.com/#feed");
  });
  ...
  it("has all section toggles defined as booleans", () => {
    const requiredSections = ["hero", "about", "services", "portfolio", "gallery", "testimonials", "contact"] as const;
